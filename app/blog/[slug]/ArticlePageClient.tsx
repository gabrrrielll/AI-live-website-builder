...
interface ArticlePageClientProps {
    ...
    slug?: string; // Slug-ul articolului din URL
}
...
export default function ArticlePageClient({ article, siteConfig, slug }: ArticlePageClientProps) {
    ...
    // Dacă articolul nu este furnizat, încarcă din localStorage folosind slug-ul pentru a găsi ID-ul
    ...
    useEffect(() => {
        if (!article && slug) {
            const loadArticle = async () => {
                try {
                    // Găsește ID-ul articolului bazat pe slug din localStorage
                    const localConfig = localStorage.getItem('site-config');
                    if (localConfig) {
                        const config = JSON.parse(localConfig);
                        setLoadedSiteConfig(config);

                        // Găsește articolul după slug pentru a obține ID-ul
                        const articleBySlug = config.articles?.find((a: Article) => a.slug === slug);
                        if (articleBySlug) {
                            const articleId = articleBySlug.id;
                            // Acum caută articolul după ID
                    ...
                    // If not found, check if it's an auto-generated article
                    ...
                    if (autoGeneratedMatch) {
                        ...
                        const autoArticle: Article = {
                            ...
                            slug: `articol-auto-generat-${articleNumber}`,
                            title: {
                            ...
                            content: {
                                ro: `<p>Conținut auto-generat pentru articolul ${articleNumber}.</p>`,
