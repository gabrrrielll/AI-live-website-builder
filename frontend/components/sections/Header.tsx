...
export const Header: React.FC<HeaderProps> = ({ sectionId }) => {
                ...
                return (
                    <div key={item.id} className="relative group/nav-item">
                        <a
                            href={href}
                            onClick={handleClick}
                            className={`text-gray-600 hover:text-[#c29a47] transition-colors
    ...
    return (
        <header className={`bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-40 relative group ${headerSection && !headerSection.visible && isEditMode ? 'opacity-50' : ''}`}>
            ...
            <div ref={dynamicElementsMeasureRef} className="absolute invisible -top-96 flex items-center">
            ...
            <nav ref={navContainerRef} className="container mx-auto px-4 sm:px-6 py-4 flex justify-between items-center min-w-0">
                <div ref={logoRef} className="flex-shrink-0 min-w-0">
